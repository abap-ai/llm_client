...
CLASS lcl_app IMPLEMENTATION.
  ...
  METHOD initialize_text_editors.
    IF text_editor_input IS NOT BOUND.  " Only initialize if not already done
      CREATE OBJECT custom_container_in EXPORTING container_name = 'INPUT_CONTAINER'.
      CREATE OBJECT text_editor_input EXPORTING parent = custom_container_in.
    ...
    IF text_editor_output IS NOT BOUND.
      CREATE OBJECT custom_container_out EXPORTING container_name = 'OUTPUT_CONTAINER'.
      CREATE OBJECT text_editor_output EXPORTING parent = custom_container_out.
  ...
  METHOD get_response.
    DATA texts TYPE STANDARD TABLE OF char255.
        ...
        DATA response TYPE zllm_response.
    ...
    TRY.
        ...
        response = client->execute( user_message = user_message ).
  ...
  METHOD set_output_text.
    ...
    DATA remaining_text LIKE text.
      DATA temp1 TYPE i.
      DATA line TYPE string.
    remaining_text = text.
    WHILE strlen( remaining_text ) > 0.
      
      IF strlen( remaining_text ) > 255.
        temp1 = 255.
      ELSE.
        temp1 = strlen( remaining_text ).
      ENDIF.
      
      line = substring( val = remaining_text
                              len = temp1 ).
